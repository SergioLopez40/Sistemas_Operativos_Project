...
void guardar(struct dogType *mascota){
   ...
   //sem_wait(&scratch_dataDog);
   //read(pipe_dataDog[0],&c,1);
   //pthread_mutex_lock(&prowrite_dog);

   
   r = fwrite(mascota, sizeof(struct dogType), 1, ap); // Se guarda una estructura en el archivo
   //pthread_mutex_unlock(&prowrite_dog);
   //write(pipe_dataDog[1],&c,1);
   //sem_post(&scratch_dataDog);
   if (r != 1){
      perror("No se pudo escribir");
      exit(-1);
   }
...
printf("Voy a escribirK\n" );
	 ...
	 /*Vamos a enviar el txt*/
		 ...
		 printf("SE ENVIO\n" );
		// pthread_mutex_lock(&prowrite_dog);
        // hacer esto pero en el archivo de la historia clinica
...
int main(){
   pthread_mutex_init(&prowrite_dog,NULL);
   int r,clientefd;
   ...
   while (run){
	...
	r=pthread_create(&hilos[clientesActuales],NULL,(void*)atender,cli); //TODO
	if (r != 0 ){
